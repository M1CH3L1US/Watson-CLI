...
export class NewAction extends AbstractAction {
  public async handle(inputs: Input[], options: Input[]) {
    ...
    if (!isDryRunEnabled) {
      ...
      console.info(chalk.green("Thank you for installing Watson 🎉"));
    }
    process.exit(0);
  }
}
...
const askForMissingInformation = async (inputs: Input[]) => {
  ...
  if (!nameInput!.value) {
    ...
    const questions = [generateInput("name", message)("watson-app")];
...
const generateApplicationFiles = async (args: Input[], options: Input[]) => {
  ...
  const collection: AbstractCollection = CollectionFactory.create(
    (collectionName as Collection) || Collection.WATSONJS
  );
 ...
 * `.gitignore` available in `@watsonjs/schematics` cannot be published to
